...
const ALLOWED_ORIGINS = ["https://dehot.bbboi.ir", "https://appassets.androidplatform.net"];
...
// ================= «حس تصویری»: ویدیوی کوتاه/گیف یا عکسِ اجباریِ همراهِ آپلودِ آهنگ =================
// جدا از audio_thumb (کاور) و audio_feeling (متن). فقط برای پست‌های نوع audio پر می‌شه. ویدیوها
// سمتِ کلاینت (بدونِ سرور) به gif تبدیل می‌شن و اینجا با sendAnimation فرستاده می‌شن — تلگرام خودش
// گیف رو به mp4 بی‌صدا تبدیل می‌کنه، برای همینه که audio_visual_feel_animated لازمه (فرانت با همون
// فلگ تصمیم می‌گیره <video> نشون بده یا <img>). این دو ستون رو هم (یک‌بار، توی کنسول D1) اضافه کن:
//   ALTER TABLE posts ADD COLUMN audio_visual_feel TEXT;
//   ALTER TABLE posts ADD COLUMN audio_visual_feel_animated INTEGER NOT NULL DEFAULT 0;
...
// ================= ایندکس‌های پیشنهادیِ عملکرد روی جدول‌های اصلی/قدیمی =================
// این ستون‌ها تویِ بیشترین کوئری‌های پرتکرارِ کل پروژه (فید، فوریو، پروفایل، چت) توی WHERE/JOIN
// استفاده می‌شن. اگه هرکدوم قبلاً روی D1 اجرا نشده (مثلاً چون جدول‌های اصلی از خیلی وقتِ پیش،
// قبل از این‌که این فایل رو نگه‌داریم، ساخته شدن)، یک‌بار توی کنسول D1 اجرا کن — بدونشون SQLite
// مجبوره برای هر کوئری کلِ جدول رو خط‌به‌خط بگرده (full table scan)، با وجودشون مستقیم می‌ره سراغِ
// ردیف‌های لازم. این تنها تغییری‌ه که خودِ کد نمی‌تونه انجامش بده (D1 دسترسیِ مستقیم به دیتابیس
// نمی‌ده)، ولی به‌تنهایی معمولاً بیشترین تأثیر رو رویِ سرعتِ کلِ سایت داره:
//   CREATE INDEX IF NOT EXISTS idx_posts_username ON posts (username);
//   CREATE INDEX IF NOT EXISTS idx_posts_type_date ON posts (type, date DESC);
//   CREATE INDEX IF NOT EXISTS idx_posts_date ON posts (date DESC);
//   CREATE INDEX IF NOT EXISTS idx_votes_username_post ON votes (username, post_id);
//   CREATE INDEX IF NOT EXISTS idx_votes_post ON votes (post_id);
//   CREATE INDEX IF NOT EXISTS idx_likes_username_post ON likes (username, post_id);
...
//   CREATE INDEX IF NOT EXISTS idx_chat_conv_members_conv ON chat_conversation_members (conversation_id);
//   CREATE INDEX IF NOT EXISTS idx_chat_conv_members_user ON chat_conversation_members (username);
//   CREATE INDEX IF NOT EXISTS idx_chat_blocks_blocker ON chat_blocks (blocker_username);
//   CREATE INDEX IF NOT EXISTS idx_chat_blocks_blocked ON chat_blocks (blocked_username);
//   CREATE INDEX IF NOT EXISTS idx_profiles_username ON profiles (username);
//   CREATE INDEX IF NOT EXISTS idx_user_presence_username ON user_presence (username);
//   CREATE INDEX IF NOT EXISTS idx_deel_views_username ON deel_views (username);
// نکته: اگه هرکدوم از این ستون‌ها همین الان PRIMARY KEY یا UNIQUE باشه، خودِ SQLite از قبل رویِ
// اون ایندکس داره — اجرای دوباره‌ی CREATE INDEX IF NOT EXISTS رویِ اونا کاملاً بی‌خطره و رد می‌شه.
...
const CAPTCHA_DIFFICULTY_BITS = 13; // هرچی بیشتر، سخت‌تر و کندتر (قبلاً ۱۸ بود؛ رو گوشی‌های معمولی چند ثانیه تا حتی بیشتر طول می‌کشید. با ۱۳، میانگینِ تلاش لازم ~۸۱۹۲ هست، یعنی معمولاً زیرِ یه ثانیه حل می‌شه و همچنان جلوی اسکریپت‌های ساده رو می‌گیره)
...
async function getUserFromTokenOrQuery(request, env) {
  ...
  // مثلِ getUserFromToken: به‌جایِ دو کوئریِ جدا (sessions بعد users)، با یه JOIN یکی شده. این
  // تابع مسیرِ اصلیِ احرازهویتِ handleMedia هست (هر عکس/آهنگ/ویدیوی هر پست)، پس همین ادغام تنهایی
  // رویِ سرعتِ لودِ مدیا (پرتکرارترین نوعِ درخواستِ کل سایت) به‌طورِ محسوسی اثر می‌ذاره.
  const row = await env.D1.prepare(
    "SELECT sessions.expires_at AS expires_at, users.username AS username, users.banned AS banned " +
    "FROM sessions JOIN users ON users.username = sessions.username " +
    "WHERE sessions.token = ?"
...
// #endregion
// #region توزیعِ تصادفیِ بارِ درخواست‌های تلگرام بینِ چند بات (اختیاری، تا ۴ تا)
// ---------- توزیعِ تصادفیِ بارِ درخواست‌های تلگرام بینِ چند بات (اختیاری، تا ۴ تا) ----------
// تلگرام به یه چتِ خاص (همون CHANNEL_ID) رو حدودِ ۲۰ پیام در دقیقه توصیه می‌کنه؛ چون همه‌چیز (پست،
// عکس، ویدیو، آهنگ، اعلانِ کامنت...) به همین یه چنل می‌ره، ممکنه به این سقف بخوریم. راه‌حل: تا ۳ بات
// کمکیِ دیگه (env.BOT_TOKEN_2 تا env.BOT_TOKEN_4)، هرکدوم به‌عنوانِ ادمینِ «ارسالِ پیام» به چنل اضافه
// می‌شن، و بارِ درخواست‌ها بینِ همه‌ی بات‌هایی که توکن‌شون تو env ست شده تصادفی پخش می‌شه. هر کدوم از
// این متغیرها ست نشده باشه نادیده گرفته می‌شه — یعنی می‌شه با ۱ تا ۴ بات کار کرد، کاملاً اختیاریه.
//
// نکته‌ی مهمِ فنی #۱: تلگرام اجازه نمی‌ده یه بات پیامی که باتِ دیگه فرستاده رو ویرایش/حذف کنه
// (editMessageCaption/editMessageText/deleteMessage فقط رویِ پیام‌های خودِ همون بات کار می‌کنه)، و
// یه فایل‌آیدی که یه بات گرفته رو نمی‌شه با باتِ دیگه دانلود کرد. برای همین:
// - هر فایلی که آپلود می‌شه، شناسه‌ش با پیشوندِ «N:» (شماره‌ی بات، ۲ تا ۴) علامت‌گذاری می‌شه اگه بات
//   غیر از باتِ اول فرستاده باشدش (tagFileId این کار رو خودکار انجام می‌ده)؛ موقعِ نمایش/دانلود
//   (handleMedia) همین پیشوند خونده می‌شه تا با باتِ درست از تلگرام گرفته بشه. باتِ اول (پیش‌فرض،
//   بدونِ پیشوند) عیناً مثلِ قبل می‌مونه تا فایل‌آیدی‌های قدیمیِ ذخیره‌شده تو D1 خراب نشن.
// - برای پست‌ها (که message_id‌شون ممکنه بعداً نیاز به ویرایشِ کپشن یا حذف داشته باشه)، همون ستونِ
//   bot_slot تو جدولِ posts (که از اول INTEGER بوده، نیازی به تغییرِ اسکیما نیست) شماره‌ی بات رو نگه
//   می‌داره: ALTER TABLE posts ADD COLUMN bot_slot INTEGER NOT NULL DEFAULT 1;
//
// نکته‌ی مهمِ فنی #۲ (هشدار، نه یه چیزِ خودکار): اگه یه‌روزی خواستی یکی از این توکن‌ها رو کامل حذف
// کنی (نه فقط از ادمینیِ چنل درش بیاری، بلکه خودِ متغیرِ BOT_TOKEN_N رو از تنظیماتِ ورکر پاک کنی)،
// هر فایلی که قبلاً با اون بات آپلود شده (یعنی تو D1 با همون پیشوند/bot_slot ذخیره شده) دیگه قابلِ
// دانلود نیست — چون فایل‌آیدیِ تلگرام مخصوصِ همون باته و بات‌های دیگه نمی‌تونن باهاش کاری کنن. پس:
// توکن‌ها رو نگه دار، فقط اگه لازم شد از ادمینیِ چنل درشون بیار (اون مشکلی برای دانلودِ فایل‌های
// قدیمی ایجاد نمی‌کنه، فقط جلوی ارسالِ پیامِ جدید با اون بات رو می‌گیره).
const TELEGRAM_BOT_ENV_KEYS = ["BOT_TOKEN", "BOT_TOKEN_2", "BOT_TOKEN_3", "BOT_TOKEN_4"];

// لیستِ { token, slot } برای هر باتی که توکنش تو env ست شده (باتِ اول/slot=۱ همیشه لازمه و همیشه تو لیسته)
function availableTelegramBots(env) {
  const list = [];
  for (let i = 0; i < TELEGRAM_BOT_ENV_KEYS.length; i++) {
    const token = env[TELEGRAM_BOT_ENV_KEYS[i]];
    if (token) list.push({ token, slot: i + 1 });
  }
  return list;
}

function pickTelegramBot(env) {
  const bots = availableTelegramBots(env);
  return bots[Math.floor(Math.random() * bots.length)];
}

function telegramTokenForSlot(env, slot) {
  // اگه توکنِ اون اسلات (مثلاً چون بعداً حذف شده) موجود نبود، به‌جای کرش، به باتِ اول برمی‌گردیم —
  // دانلود همچنان ۴۰۴ می‌شه (چون فایل‌آیدی مالِ باتِ دیگه‌ست) ولی درخواست کرش نمی‌کنه.
  return env[TELEGRAM_BOT_ENV_KEYS[(slot || 1) - 1]] || env.BOT_TOKEN;
}

// شناسه‌ی فایل رو (اگه باتی غیر از باتِ اول فرستاده باشدش) با پیشوندِ «N:» علامت‌گذاری می‌کنه تا موقعِ
// دانلود بشه فهمید با کدوم بات باید از تلگرام گرفته بشه. شناسه‌های قدیمی/بدونِ پیشوند یعنی باتِ اول.
function tagFileId(id, slot) {
  if (!id) return id || null;
  return slot && slot > 1 ? `${slot}:${id}` : id;
}

// پیشوندِ احتمالیِ «N:» (N بینِ ۲ تا ۴) رو از یه فایل‌آیدی جدا می‌کنه؛ برمی‌گردونه { id, slot }
function untagFileId(fileId) {
  if (typeof fileId === "string") {
    const m = /^([234]):(.+)$/.exec(fileId);
    if (m) return { id: m[2], slot: Number(m[1]) };
  }
...
async function handlePost(request, env) {
  ...
  // «حس تصویری»: ویدیوی کوتاه (که کلاینت به gif تبدیلش کرده) یا عکس؛ برای آهنگ اجباریه (چک پایین‌تر)
  const audioVisualFeelFile = form.get("audio_visual_feel");
  const hasAudioVisualFeel = audioVisualFeelFile && typeof audioVisualFeelFile !== "string" && audioVisualFeelFile.size > 0
    && audioVisualFeelFile.size <= 12 * 1024 * 1024;
  const clientAudioVisualFeelAnimated = ["1", "true", "on"].includes((form.get("audio_visual_feel_animated") || "").toString().trim().toLowerCase());
  ...
  // «حس تصویری» اجباریه: بدونِ اون اصلاً وارد فازِ آپلودِ واقعی (تلگرام) نمی‌شیم
  if (isAudioUpload && !hasAudioVisualFeel) {
    return json({ error: "برای آهنگ باید یه ویدیوی کوتاه یا عکس برای «حس تصویری» انتخاب کنی" }, 400);
  }
  // نکته: حتی برای حالتِ «انیمیت» هم فایلِ واقعی یه gif/image خام‌ه (کلاینت خودش تبدیلش کرده)؛
  // فلگِ animated فقط تعیین می‌کنه با sendAnimation فرستاده بشه یا sendPhoto، نه اینکه امضایِ فایل چیه
  if (hasAudioVisualFeel && !(await verifyFileMatchesCategory(audioVisualFeelFile, "image"))) {
    return json({ error: "فایلِ «حس تصویری» نامعتبره" }, 400);
  }
  ...
  let audioVisualFeelFileId = null;
  let drawingFileId = null;

  // آپلودِ «حس تصویری» به تلگرام: مستقل از این‌که مسیرِ آهنگ preUploaded باشه یا مستقیم، چون فقط
  // به نوعِ پست (audio) وابسته‌ست نه به مسیرِ آپلودِ خودِ آهنگ. گیف با sendAnimation می‌ره (تلگرام
  // خودش تبدیلش می‌کنه به mp4 بی‌صدا)، عکس با sendPhoto و در صورتِ شکست، sendDocument.
  const uploadAudioVisualFeel = async () => {
    if (!hasAudioVisualFeel) return;
    try {
      if (clientAudioVisualFeelAnimated) {
        const animResult = await sendTelegramFile(env, "sendAnimation", "animation", audioVisualFeelFile, `حس تصویری — ${username}`);
        audioVisualFeelFileId = extractFileId("animation", animResult);
      } else {
        const photoResult = await sendTelegramFile(env, "sendPhoto", "photo", audioVisualFeelFile, `حس تصویری — ${username}`);
        audioVisualFeelFileId = extractFileId("photo", photoResult);
      }
    } catch (visualErr) {
      try {
        const docResult = await sendTelegramFile(env, "sendDocument", "document", audioVisualFeelFile, `حس تصویری — ${username}`);
        audioVisualFeelFileId = extractFileId("document", docResult);
      } catch (docErr) {
        console.error("خطای آپلودِ «حس تصویری» (هم مسیرِ اصلی هم سند شکست خورد):", visualErr.message, "|", docErr.message);
      }
    }
  };
  ...
  try {
    ...
    // «حس تصویری»: مستقل از این‌که مسیرِ آهنگ preUploaded بوده یا مستقیم، فقط به type=='audio' وابسته‌ست
    if (type === "audio") {
      await uploadAudioVisualFeel();
    }
  ...
  const post = {
    ...
    audio_thumb: null,
    audio_feeling: null,
    audio_visual_feel: null,
    audio_visual_feel_animated: 0,
    video_thumb: null,
    tags: tagsJson,
  ...
  if (type === "audio") {
    ...
    post.audio_visual_feel = audioVisualFeelFileId || null;
    post.audio_visual_feel_animated = clientAudioVisualFeelAnimated && audioVisualFeelFileId ? 1 : 0;
  ...
  try {
    await bind(
      env.D1.prepare(
        `INSERT INTO posts (id, username, text, title, type, file_id, message_id, bot_slot, date, upvotes, downvotes, likes, audio_title, audio_performer, audio_thumb, audio_feeling, audio_visual_feel, audio_visual_feel_animated, video_thumb, tags, auto_tags, duration_seconds, radio_visual, drawing_file_id, audio_game)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ),
      [post.id, post.username, post.text, post.title, post.type, post.file_id, post.message_id, post.bot_slot, post.date, post.audio_title, post.audio_performer, post.audio_thumb, post.audio_feeling, post.audio_visual_feel, post.audio_visual_feel_animated, post.video_thumb, post.tags, post.auto_tags, post.duration_seconds || null, post.radio_visual, post.drawing_file_id, post.audio_game || null]
    ).run();
  } catch (err) {
...
// همون استراتژیِ بالا، برایِ ویدیو هم به کار می‌ره. قبلاً این‌کار فقط مالِ صدا بود چون نگرانیِ اصلی
// بافرکردنِ کاملِ یه فایلِ بزرگ تو حافظه‌ی محدودِ ورکر (۱۲۸ مگابایت) بود؛ ولی چون آپلودِ ویدیو تو
// همین ورکر از قبل به ۵۰ مگابایت محدوده (نگاه کن به maxSize تو هندلرهایِ آپلود)، حتی بدترین حالت
// (چند تیکه‌ی هم‌زمان + بافرِ نهاییِ چسبیده‌شده هم‌زمان تو حافظه) حدودِ ۱۰۰ مگابایت می‌شه — هنوز
// زیرِ سقف. برایِ همین امن‌ه که ویدیو هم دقیقاً مثلِ صدا رفتار کنه: اولین پخش با چند اتصالِ موازی
// سریع‌تر میاد، و پخش/سیکِ بعدی (همون کاربر یا کاربرهایِ دیگه) کلاً از کشِ edge کلادفلر سرو می‌شه،
// بدونِ تماسِ دوباره با سرورِ (نسبتاً کندِ) فایلِ تلگرام.
const VIDEO_FILE_EXT_REGEX = /\.(mp4|m4v|mov|webm|mkv|3gp|3gpp|avi|ts)$/i;
...
// حداکثر تعداد تیکه‌های هم‌زمان؛ هر تیکه تقریباً ۱ مگابایته (عددِ کمتر برای فایل‌هایِ کوچیک‌تر خودکار
// انتخاب می‌شه). روی ۶ تنظیم شده، نه بیشتر، چون سقفِ واقعیِ ورکرهایِ کلادفلر برایِ تعدادِ اتصالِ
// هم‌زمانِ خروجی در هر درخواست همینه؛ عددِ بزرگ‌تر فقط صف می‌کشه، سرعتِ واقعی اضافه نمی‌کنه.
const PARALLEL_FETCH_MAX_CHUNKS = 6;
...
async function handleMedia(fileId, env, request, ctx) {
  ...
  const isFastCacheableFile = AUDIO_FILE_EXT_REGEX.test(filePath || "") || VIDEO_FILE_EXT_REGEX.test(filePath || "");

  if (isFastCacheableFile) {
    // برایِ فایل‌هایِ صوتی/ویدیویی، کلِ فایل رو (با چند تیکه‌ی موازی اگه به‌قدرِ کافی بزرگ بود — نگاهِ
    // بالا، fetchTelegramFileFast) یه‌جا می‌گیریم، کاملش رو کش می‌کنیم، و همون بازه‌ای که مرورگر
    // خواسته رو از رویِ همین جوابِ تازه می‌بُریم. این‌جوری اولین پخشِ هر آهنگ/ویدیو چندبرابر سریع‌تر
    // از قبله (چند اتصالِ هم‌زمان به‌جایِ یه اتصالِ سریالِ کند)، و هر پخش/سیکِ بعدی (همین کاربر یا
    // کاربرهایِ دیگه) کلاً از edge کشِ کلادفلر میاد، بدونِ هیچ تماسی با تلگرام
  ...
  // بقیه‌ی فایل‌ها (عکس/سند/استیکر و...): همون رفتارِ قبلی — بازه‌ی خواسته‌شده رو عیناً از تلگرام
  // می‌خوایم، نه کلِ فایل. این‌ها معمولاً یا خیلی کوچیکن (عکس/استیکر، که خودِ مرورگر با هدرِ
  // Cache-Control پایین‌تر کشش می‌کنه) یا نوعشون نامشخصه (سند)، پس صرفِ اجراییِ استراتژیِ بالا رو ندارن
...
async function handleChatMembers(request, env) {
  ...
  const avatarMap = {};
  const presenceMap = {};
  // بهینه‌سازی: قبلاً برایِ آنلاین‌بودنِ هر عضو (تا ۵۰ نفر) یه کوئریِ جدا زده می‌شد؛ الان با یه
  // batch مشترک (کنارِ کوئریِ آواتارها) گرفته می‌شه.
  if (usernames.length) {
    const placeholders = usernames.map(() => "?").join(",");
    const [profRows, presenceRows] = await env.D1.batch([
      bind(env.D1.prepare(`SELECT username, avatar_file_id FROM profiles WHERE username IN (${placeholders})`), usernames),
      bind(env.D1.prepare(`SELECT username, last_active_at FROM user_presence WHERE username IN (${placeholders})`), usernames),
    ]);
    for (const row of profRows.results || []) {
      if (row.avatar_file_id) avatarMap[row.username] = row.avatar_file_id;
    }
    for (const row of presenceRows.results || []) {
      presenceMap[row.username] = row.last_active_at;
    }
  }

  const members = (rows.results || []).map((row) => {
    const lastActiveAt = presenceMap[row.username] || null;
    return {
...
// بهینه‌سازیِ مهم: نسخه‌ی قبلی برایِ هر گفتگو (تا ۱۰۰ تا) ۲ تا ۶ کوئریِ سریالیِ جدا به D1 می‌زد —
// یعنی برایِ کاربری با ۲۰ گفتگو، تا ۹۰+ رفت‌وبرگشتِ پشتِ‌سرِهم فقط برای باز کردنِ تبِ چت (که هم
// اولین‌بار و هم با هر پولینگ صدا زده می‌شه). الان همه‌چی با ۲ تا D1.batch (که هرکدوم صرفِ‌نظر از
// تعدادِ کوئریِ داخلش فقط یه رفت‌وبرگشتِ شبکه‌ست) جمع می‌شه: فازِ ۱ (پارتنرِ گفتگوهای دونفره +
// تعدادِ اعضایِ گروه‌ها + آخرین پیام + تعدادِ نخوانده‌ی هر گفتگو)، فازِ ۲ (بلاک/آواتار/آنلاین‌بودنِ
// همون پارتنرها که فازِ ۱ مشخص کرد).
async function handleChatList(request, env) {
  ...
  let convRows = rows.results || [];

  // گفتگویِ «پیام‌های ذخیره‌شده» فقط دفعه‌ی اولِ هر کاربر ساخته نشده؛ به‌جایِ چک‌کردنِ همیشگیِ وجودش
  // با یه رفت‌وبرگشتِ اضافه (رفتارِ قبلی)، فقط وقتی تویِ نتیجه‌ی بالا نبود می‌سازیمش — یعنی برایِ
  // تقریباً همه‌ی درخواست‌های بعدِ اولین بار، این هزینه کلاً حذف می‌شه.
  if (!convRows.some((r) => r.type === "saved")) {
    const savedId = await ensureSavedMessagesConversation(env, username);
    convRows = [
      { id: savedId, type: "saved", title: SAVED_MESSAGES_TITLE, last_message_at: null, avatar_file_id: null, last_read_at: 0, muted: 0 },
      ...convRows,
    ];
  }

  if (convRows.length === 0) return json({ conversations: [] });

  const directIds = convRows.filter((r) => r.type === "direct").map((r) => r.id);
  const groupIds = convRows.filter((r) => r.type !== "direct" && r.type !== "saved").map((r) => r.id);
  ...
  if (directIds.length > 0) {
    phase1Keys.push("partners");
    phase1Stmts.push(bind(
      env.D1.prepare(`SELECT conversation_id, username FROM chat_conversation_members WHERE conversation_id IN (${directIds.map(() => "?").join(",")}) AND username != ?`),
      [...directIds, username]
    ));
  }
  if (groupIds.length > 0) {
    phase1Keys.push("groupCounts");
    phase1Stmts.push(bind(
      env.D1.prepare(`SELECT conversation_id, COUNT(*) AS c FROM chat_conversation_members WHERE conversation_id IN (${groupIds.map(() => "?").join(",")}) GROUP BY conversation_id`),
      groupIds
    ));
  }
  phase1Keys.push("lastMsgs");
  phase1Stmts.push(bind(
    env.D1.prepare(
      `SELECT conversation_id, sender_username, msg_type, text, created_at, deleted_at FROM (
         SELECT *, ROW_NUMBER() OVER (PARTITION BY conversation_id ORDER BY created_at DESC) AS rn
         FROM chat_messages WHERE conversation_id IN (${idPh})
       ) WHERE rn = 1`
    ),
    allIds
  ));
  {
    // آستانه‌ی «خونده‌شده»ی هر گفتگو فرق می‌کنه (last_read_at خودِ اون عضویته)؛ با یه CASE داخلِ خودِ
    // SQL این تفاوت رو بدونِ نیاز به کوئریِ جدا برای هر ردیف اعمال می‌کنیم.
    const caseSql = convRows.map(() => "WHEN ? THEN ?").join(" ");
    const caseParams = [];
    convRows.forEach((r) => caseParams.push(r.id, r.last_read_at || 0));
    phase1Keys.push("unread");
    phase1Stmts.push(bind(
      env.D1.prepare(
        `SELECT conversation_id, COUNT(*) AS c FROM chat_messages
         WHERE conversation_id IN (${idPh}) AND sender_username != ? AND deleted_at IS NULL
           AND created_at > (CASE conversation_id ${caseSql} ELSE 9223372036854775807 END)
         GROUP BY conversation_id`
      ),
      [...allIds, username, ...caseParams]
    ));
  }

  const phase1Results = await env.D1.batch(phase1Stmts);
  const byKey = {};
  phase1Keys.forEach((key, i) => { byKey[key] = phase1Results[i]; });

  const partnerMap = {};
  for (const row of (byKey.partners && byKey.partners.results) || []) partnerMap[row.conversation_id] = row.username;
  const groupCountMap = {};
  for (const row of (byKey.groupCounts && byKey.groupCounts.results) || []) groupCountMap[row.conversation_id] = row.c;
  const lastMsgMap = {};
  for (const row of (byKey.lastMsgs && byKey.lastMsgs.results) || []) lastMsgMap[row.conversation_id] = row;
  const unreadMap = {};
  for (const row of (byKey.unread && byKey.unread.results) || []) unreadMap[row.conversation_id] = row.c;

  // فازِ ۲: حالا که پارتنرِ همه‌ی گفتگوهای دونفره مشخصه، بلاک/آواتار/آنلاین‌بودنشون رو یه‌جا می‌گیریم
  const otherUsernames = [...new Set(Object.values(partnerMap).filter(Boolean))];
  const blockMap = {};
  const avatarMap = {};
  const presenceMap = {};
  if (otherUsernames.length > 0) {
    const otherPh = otherUsernames.map(() => "?").join(",");
    const [blockRes, avatarRes, presenceRes] = await env.D1.batch([
      bind(env.D1.prepare(`SELECT blocked_username FROM chat_blocks WHERE blocker_username = ? AND blocked_username IN (${otherPh})`), [username, ...otherUsernames]),
      bind(env.D1.prepare(`SELECT username, avatar_file_id FROM profiles WHERE username IN (${otherPh})`), otherUsernames),
      bind(env.D1.prepare(`SELECT username, last_active_at FROM user_presence WHERE username IN (${otherPh})`), otherUsernames),
    ]);
    for (const row of blockRes.results || []) blockMap[row.blocked_username] = true;
    for (const row of avatarRes.results || []) avatarMap[row.username] = row.avatar_file_id;
    for (const row of presenceRes.results || []) presenceMap[row.username] = row.last_active_at;
  }
  ...
  const conversations = convRows.map((row) => {
    ...
    let blockedByMe = false;
    let avatarFileId = row.avatar_file_id || null;
    let memberCount = null;
    let online = false;
    let lastActiveAt = null;

    if (row.type === "direct") {
      otherUsername = partnerMap[row.id] || null;
      displayTitle = otherUsername || "کاربر حذف‌شده";
      if (otherUsername) {
        blockedByMe = !!blockMap[otherUsername];
        avatarFileId = avatarMap[otherUsername] || null;
        lastActiveAt = presenceMap[otherUsername] || null;
        online = !!lastActiveAt && Date.now() - lastActiveAt < ONLINE_WINDOW_MS;
      }
    } else if (row.type === "saved") {
      displayTitle = SAVED_MESSAGES_TITLE;
    } else {
      memberCount = groupCountMap[row.id] || 0;
    }

    const lastMsg = lastMsgMap[row.id] || null;
...
async function handleChatForwardMessage(request, env) {
  ...
  // بهینه‌سازی: قبلاً برایِ هر مقصد (تا ۱۵ تا) ۲ کوئریِ چک + ۳ کوئریِ نوشتنِ کاملاً سریال اجرا می‌شد
  // (تا ۷۵ رفت‌وبرگشتِ D1 برایِ یه فوروارد به ۱۵ گفتگو). حالا چک‌های عضویت/نوعِ همه‌ی مقصدها با یه
  // batch مشترک انجام می‌شه، و نوشتن‌های همه‌ی مقصدهایِ معتبر هم با یه batchِ دوم.
  const forwardedFromLabel = original.sender_username;
  const targetPh = targetConversationIds.map(() => "?").join(",");
  const [memberRes, convRes] = await env.D1.batch([
    bind(
      env.D1.prepare(`SELECT conversation_id FROM chat_conversation_members WHERE username = ? AND conversation_id IN (${targetPh})`),
      [username, ...targetConversationIds]
    ),
    bind(
      env.D1.prepare(`SELECT id, type FROM chat_conversations WHERE id IN (${targetPh})`),
      targetConversationIds
    ),
  ]);
  const memberSet = new Set((memberRes.results || []).map((r) => r.conversation_id));
  const convTypeMap = {};
  for (const row of convRes.results || []) convTypeMap[row.id] = row.type;

  // ترتیبِ اصلیِ درخواستِ کاربر رو حفظ می‌کنیم؛ فقط مقصدهایی که واقعاً عضوشونیم و هنوز وجود دارن می‌مونن
  const validTargets = targetConversationIds.filter((id) => memberSet.has(id) && convTypeMap[id]);
  const now = Date.now();

  const writeStmts = [];
  for (const targetConversationId of validTargets) {
    const id = `${Date.now()}_${randomHex(4)}`;
    writeStmts.push(bind(
      env.D1.prepare(
        "INSERT INTO chat_messages (id, conversation_id, sender_username, msg_type, text, file_id, is_external, created_at, reply_to_message_id, forwarded_from, file_name, file_size) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
      ),
      [id, targetConversationId, username, original.msg_type, original.text, original.file_id, original.is_external, now, null, forwardedFromLabel, original.file_name || null, original.file_size || null]
    ));
    writeStmts.push(bind(env.D1.prepare("UPDATE chat_conversations SET last_message_at = ? WHERE id = ?"), [now, targetConversationId]));
    writeStmts.push(bind(
      env.D1.prepare("UPDATE chat_conversation_members SET last_active_at = ? WHERE conversation_id = ? AND username = ?"),
      [now, targetConversationId, username]
    ));
  }
  if (writeStmts.length > 0) await env.D1.batch(writeStmts);

  for (const targetConversationId of validTargets) {
    notifyChatMembersOfNewMessage(
      env, { type: convTypeMap[targetConversationId] }, targetConversationId, username, original.msg_type, original.text, now
    ).catch(() => {});
  }

  return json({ ok: true, sentTo: validTargets });
}
// #endregion
